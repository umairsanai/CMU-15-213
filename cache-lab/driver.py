...
def computeMissScore(miss, lower, upper, full_score):
    ...
    range_val = (upper - lower) * 1.0   # 'range' is a built-in, avoid shadowing
...
def main():
    ...
    # Fix map: convert to list for Python 3
    csim_cscore = list(map(int, resultsim[0:1])) if resultsim else [0]
    trans_cscore = int(result32[0]) * int(result64[0]) * int(result61[0]) if result32 and result64 and result61 else 0
    ...
    trans32_score = computeMissScore(miss32, 300, 600, maxscore['trans32']) * int(result32[0]) if result32 else 0
    trans64_score = computeMissScore(miss64, 1300, 2000, maxscore['trans64']) * int(result64[0]) if result64 else 0
    trans61_score = computeMissScore(miss61, 2000, 3000, maxscore['trans61']) * int(result61[0]) if result61 else 0
